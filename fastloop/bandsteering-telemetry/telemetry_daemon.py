...
import rfeqm
import random
import basic_band_steering
from ewma import frequency_aware_smoothing
...
class NetworkOrchestrator:
    ...
    def _calculate_ap_score(self, rssi, channel_utilization=None):
        ...
        # For testing purposes, randomized data has been used to calculate rfeqm data
        rfeqm_score = rfeqm.compute_rfeqm([{"type": "bluetooth", "prob": random.uniform(0.8, 1), "duty": random.random(), "power": random.random() * 1e-4}], 1e-8)
        self.rfeqm_scores.append(rfeqm_score)
        
        scores = frequency_aware_smoothing(self.rfeqm_scores, alpha_up=0.0125, alpha_down=0.45)
        rfeqm_score = scores[-1]

        if rfeqm_score >= 55:
            rfeqm_score = 0

        return score * 0.45 + 0.55 * rfeqm_score
    ...
    def start_k_steering_evaluation(self, client):
        ...
        client.supports_lmr = supports_lmr 
...
def main():
    ...
    band_steering_manager = basic_band_steering.BandSteeringManager(basic_band_steering.CONFIG)

    print("Info: Steering daemon started. Reading from stdin...", file=sys.stderr)

    try:
        while True:
            band_steering_manager.run_once(orchestrator)
