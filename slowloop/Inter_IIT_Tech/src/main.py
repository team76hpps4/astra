...
# Assuming give_input.py (2.4G MUX) is in the same directory
from give_input import generate_rrm_arrays 

# --- HELPER FUNCTION TO BUILD INITIAL CONFIGURATION (2.4G Specific) ---
def build_initial_ap_list(config):
    """
    Constructs a baseline list of 36 AP configuration dictionaries 
    using the default configuration parameters (2.4G), required for the MUX 
    to read the 'current_full_ap_configs' list during the first iteration.
    """
    
    NUM_ROOMS = int(config['num_rooms'])
    NUM_APS_PER_ROOM = int(config['ap_per_room'])
    
    # Use random settings that are within valid 2.4G ranges
    DEFAULT_TX = 18.0
    DEFAULT_PD = -70.0
    CH_CYCLE_ROOM = [1.0, 6.0, 11.0] 
    DEFAULT_CW = 20.0 # Default to 20 MHz in 2.4G
...
def main():
    ...
    # ==========================================
    # 1. BASE CONFIGURATION (2.4 GHz Specific)
    # ==========================================
    CHANNELS_2G = list(range(1, 14)) # 1 to 13
    NUM_ROOMS = 12
    NUM_APS_PER_ROOM = 3
    ...
    config = {
        ...
        'noise_variation_frac': 0.03, # Included fix
        'short_term_shadowing_sigma': 6.0, # Included fix
        'shadowing_sigma': 4.0, # Included fix
        'cca_threshold_dbm': -80.0,
        'num_rooms': float(NUM_ROOMS),
        'ap_per_room': float(NUM_APS_PER_ROOM),
        'client_grid_spacing': 5.0, 
        'room_size': matlab.double([10, 10]), # Included fix
        'channels': matlab.double(CHANNELS_2G),
        'MCS_SINR_Required': matlab.double([0,1.5,3.5,5,8,10,12,13,15,17,18,20]),
        'pathloss': {'n_exp': 3.8, 'wall_loss_db': 15.0, 'PL_d0_dB': 40.0, 'd0': 1.0},
        'MCS_Rates_20MHz': matlab.double([6.5,13,19.5,26,39,52,58.5,65,78,86.7,97.5,108.3]), 
    ...
    # ==========================================
    # Call the helper function to build the initial list of 36 AP dictionaries
    static_ap_template = build_initial_ap_list(config)
    
    # The MUX needs the full list from the previous step (T=0)
    current_full_ap_configs = copy.deepcopy(static_ap_template)
    
    print(f"--- 2.4 GHz SIMULATION START ---")
    ...
    for step in range(TOTAL_STEPS):
        ...
        for i in range(NUM_TOTAL_APS):
            ...
            # Channel cycling for 2.4 GHz (1, 6, 11) is essential for co-channel interference.
            start_ch = float(room_channel[r])
            CH_CYCLE_ROOM = [1.0, 6.0, 11.0] 
            
            # Find the index of the committed channel to start the cycle
            if start_ch in CH_CYCLE_ROOM:
                 start_idx = CH_CYCLE_ROOM.index(start_ch)
            else:
                 start_idx = 0 
            
            # Cycle through non-overlapping channels starting at the committed channel
            ap_struct['channel'] = CH_CYCLE_ROOM[(start_idx + k) % len(CH_CYCLE_ROOM)]
        ...
        t0 = time.time()
        print(f"Executing 2.4G simulation for {NUM_TOTAL_APS} APs...", flush=True)
        
        # IMPORTANT: Call the 2.4 GHz MATLAB function
        eng.eval("res = simulate_environment(config, ap_configs);", nargout=0)
        ...
        # Note: 2.4G matrix size is 59 rows. P95 Thr is Row 56, P95 Retry is Row 57 (Python index)
        ...
        # 3. Extract P50 Throughput (Row 56 - P95 Throughput in MATLAB is 57)
        # Note: Using Row 56 as a proxy for P50, since P95 is often used for throughput aggregation
        p50_thr_per_ap = net1_matrix[56, :] 
        current_mean_p50_thr = np.mean(p50_thr_per_ap)
        
        # 4. Extract P95 Retry Rate (Row 57 in Python indexing)
        p95_retry_per_ap = net1_matrix[57, :]
        ...
        all_p95_retry_per_ap.append(p95_retry_per_ap) 
