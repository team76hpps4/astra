...
from give_input import generate_rrm_arrays # 2.4G MUX
from planner_2 import NUM_NETWORK_1_APS, RL_SPARSE_SIZE

# ==========================================
# 1. CONFIGURATION CONSTANTS (2.4 GHz Specific)
# ==========================================
NUM_ROOMS = 12
NUM_APS_PER_ROOM = 3
...
config_2g = {
    'random_seed': 123,
    'dt_ms': 300000.0,
    'channel_width_mhz': 40.0, 'noise_figure_db': 7.0, 'noise_floor_dbm': -95.0,
    ...
    'channels': matlab.double(list(range(1, 14))),
    ...
    'sim_duration_s': 10.0, 
    'rl_episode_duration_min': 240.0, 
}

# ----------------------------------------------------------------------


def run_2g_step(eng, config, current_full_ap_configs, static_ap_template, step, SELECT, *aggregation_history):
    """ 
    Executes one time step of the 2.4 GHz simulation and RRM loop. 
    """
    
    step_time_s = (step + 1) * (config['dt_ms'] / 1000)
    print(f"\n[2.4G] Running Step {step + 1} (T={step_time_s:.0f} s)...", flush=True)
    ...
    print(f"[2.4G] Execution time: {time_elapsed:.2f}s", flush=True)

    # --- D. Return updated state and results ---
    return ap_configs_list, results


def get_2G_data(eng, config, current_full_ap_configs, step):
    """ 
    Executes one time step of the 2.4 GHz simulation and RRM loop for training.
    """
    
    # FIX: Define NET1_SLICE locally or pass it down. Defining locally here:
    NET1_SLICE = slice(0, None, 3) 
    
    step_time_s = (step + 1) * (config['dt_ms'] / 1000)
    #print(f"\n[2.4G] Running Step {step + 1} (T={step_time_s:.0f} s)...", flush=True)
    #log_file.write(f"\n--- STEP {step + 1} (T={step_time_s:.0f} s) [2.4G] ---\n")
    ...
    # 1. Filter Metrics (59 rows, 36 columns -> 59 rows, 12 columns)
