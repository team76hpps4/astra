...
with open("var.csv", "w") as _f:
    _f.write("timestamp,client_mac,variance_s2,uplink_bytes_per_s,downlink_bytes_per_s\n")

with open("delay.csv", "w") as _f:
    _f.write("timestamp,client_mac,delay_ms\n")



# --- Keys helper (unchanged) ---
...
class RetryMonitorPro:
    ...
    # --- parse iw/iwinfo to gather station/final stats (unchanged) ---
    ...
    # --- get_results: add ack variance stats for clients ---
    ...
    # --- capture loop: collects packets and outputs JSON every sampling_time ---
            ...
            if continuous:
                ...
                while time.time() < end_time:
                    try:
                        ...
                        pass
                # return snapshot (no clearing here - CLI will print and exit)
